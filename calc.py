...
正直に言っておく限界
  ...
  ・冷房は「温度を下げる分」だけを数えている。湿気を取る（除湿）分は入れていないので、
    冷房費は実際より低めに出る。おおむね3〜6割低めと見ている（推定）。
    織り込んでも全国の差は4.4〜4.8倍で「4倍以上」は変わらないが、
    東京の内訳「暖房は冷房のおよそ4倍」は2.3〜2.9倍まで縮む
  ・cop_heat の折れ線に「効率が落ち始める温度」は無い。1℃あたりの変化は
    0.100 / 0.111 / 0.160 / 0.113 でほぼ一定。7℃はカタログの定格を測る外気温であって、
    そこから落ち始めるわけではない
...
def annual(temps, price, days=None, bh=BASE_HEAT, bc=BASE_COOL, hloss=H_LOSS,
    ...
    for t, d in zip(temps, days or DAYS_NORMAL):
        ...
        if t > bc:
            ...
            # **cop_cool() を実際に呼ぶ。** ここで bc（度日の基準温度）を引いていたため、
            #   README に書いてある式（24℃固定）と実際の計算が違っていた。
            #   エアコンの効率は機械の性質、bc は計算上の決めごとで、本来は無関係。
            #   基準ケース（bc=24）では答えは1円も変わらないが、
            #   05の感度分析では729通り中486通りが README の式と違う値になっていた（2026-08-08 修正）
            cool_kwh += tc / cop_cool(t, cool_a)
    return {
